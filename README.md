...
- **Call hold** uses a bidirectional `<Connect><Stream>` in TwiML. The server sends ringback as mu-law media over the active WebSocket, while unlock still uses Twilio's `<Play digits="9"/>` call update.
