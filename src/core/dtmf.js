...
const DEFAULT_TONE_MS = 650;
const DEFAULT_TRAILING_SILENCE_MS = 120;
...
function createDtmfMulawTone(digit, options = {}) {
  ...
  for (let i = 0; i < toneSamples; i++) {
    ...
    const sample =
      (amplitude *
        (Math.sin(2 * Math.PI * lowFrequency * t) +
          Math.sin(2 * Math.PI * highFrequency * t))) /
      2;
 ...
 */
async function sendDtmfTone(activeCall, digit, options = {}) {
  if (!activeCall || !activeCall.streamSid || !activeCall.wsConnection) {
